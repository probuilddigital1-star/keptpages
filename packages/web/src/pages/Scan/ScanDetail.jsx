...
export default function ScanDetail() {
  ...
  const justScanned = location.state?.justScanned;
  ...
  const [showNextSteps, setShowNextSteps] = useState(!!justScanned);
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Top bar */}
      <div className="flex flex-wrap items-center gap-2 px-3 sm:px-4 py-2 sm:py-3 bg-cream-surface border-b border-border-light">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          <button
            onClick={() => navigate(backPath)}
            className="flex items-center gap-1.5 p-2 sm:p-1.5 rounded-md text-walnut-secondary hover:bg-cream-alt transition-colors shrink-0"
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 ml-auto">
          {confidence < 0.7 && (
            ...
            >
              ...
              <span className="hidden sm:inline">Reprocess with AI</span>
            </Button>
          )}
          <button
            type="button"
      ...
      {/* Next steps banner (shown after fresh scan) */}
      {showNextSteps && (
        <div className="px-4 py-3 bg-sage-light border-b border-sage/20">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div className="flex items-center gap-2 min-w-0">
              <svg className="h-5 w-5 text-sage shrink-0" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <p className="font-ui text-sm text-walnut">
                <span className="font-medium">Scan saved!</span>
                {' '}Review the text below, then:
              </p>
            </div>
            <div className="flex items-center gap-2">
              {fromCollection && (
                <Button
                  size="sm"
                  variant="secondary"
                  onClick={() => navigate(`/app/scan`, { state: { collectionId: fromCollection, collectionName: fromCollectionName } })}
                >
                  Scan Another
                </Button>
              )}
              <Button
                ...
                onClick={() => navigate(backPath)}
              >
                {fromCollection ? 'Back to Collection' : 'View All Scans'}
              </Button>
              <button
                type="button"
                onClick={() => setShowNextSteps(false)}
                className="p-1 rounded text-walnut-muted hover:text-walnut transition-colors"
                aria-label="Dismiss"
              >
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
